//! Kyber — gestionnaire de mots de passe post-quantique, 100 % local.
//!
//! Ce crate (`app_lib`) porte toute la logique métier ; il est réutilisé tel
//! quel par le binaire de l'app (`main.rs`) et par l'hôte de native messaging
//! de l'extension (`bin/native_host.rs`).
//!
//! - [`crypto`] — dérivation de clé (Argon2id + ML-KEM-1024 + HKDF) et
//!   chiffrement AES-256-GCM. **Toute la crypto est ici.**
//! - [`vault`] — modèle du coffre et formats de fichier `.vault` (v1 / v2).
//! - [`filelock`] — chiffrement de fichiers et dossiers (`.kyber`, format `KYBF`).
//! - [`scanner`] — détection du champ mot de passe actif (par OS).
//! - [`session_bridge`] — socket loopback exposant le coffre ouvert à l'extension.
//!
//! Les fonctions `#[tauri::command]` de ce fichier constituent la surface
//! appelée depuis `ui/main.js`. Voir `ARCHITECTURE.md`.
...
use std::sync::Mutex;
use tauri::State;
use vault::{EncryptedVault, EncryptedVaultV2, VaultData, VaultEntry, V2_MAGIC};
...
fn save_last_vault_path(path: &Path) {
    let _ = fs::write(config_path(), path.to_string_lossy().as_bytes());
}

// ── Registre des coffres créés (chemins connus, pour la liste au démarrage) ─
...
fn init_vault(
    ...
    if vault_path.is_dir() || path.ends_with('/') || path.ends_with('\\') {
        return Err(
            "Vous avez indiqué un dossier. Veuillez ajouter un nom de fichier.".to_string(),
        );
    }
...
fn save_vault(
    ...
    if raw.starts_with(V2_MAGIC) {
        let mut v2: EncryptedVaultV2 =
            bincode::deserialize(&raw[3..]).map_err(|_| "Coffre v2 corrompu.".to_string())?;
        v2.nonce = nonce;
        ...
        let mut v1: EncryptedVault =
            bincode::deserialize(&raw).map_err(|_| "Coffre v1 corrompu.".to_string())?;
        v1.nonce = nonce;
...
fn get_vault_health(state: State<'_, AppState>) -> Result<HealthReport, String> {
    ...
    for entry in vault.entries.values() {
        if password_score(&entry.password) < 50 {
            weak.push(entry.clone());
        }
        if now.saturating_sub(entry.last_modified) > ninety_days {
    ...
    let duplicates: Vec<VaultEntry> = pw_map
        .into_values()
        .filter(|v| v.len() > 1)
        .flatten()
        .collect();
...
fn import_csv(
    ...
    let col = |names: &[&str]| -> Option<usize> {
        names.iter().find_map(|&n| {
            headers
                .iter()
                .position(|h| h.to_lowercase() == n.to_lowercase())
        })
    };
...
fn encrypt_file_cmd(
    ...
    let mk = mk_lock
        .as_ref()
        .ok_or("Coffre non déverrouillé. Ouvrez d'abord votre coffre.")?;
...
fn migrate_to_v2(state: State<'_, AppState>, password: &str) -> Result<(), String> {
    ...
    let v1: EncryptedVault =
        bincode::deserialize(&raw).map_err(|_| "Fichier coffre invalide.".to_string())?;
